...
function generateCoinsTable(coins, mlEnabled = false) {
    ...
    coins.forEach((coin, index) => {
        ...
        let aiSentimentHtml = '';
        if (coin.ai_sentiment && coin.ai_sentiment.score !== undefined) {
            const sentiment = coin.ai_sentiment;
            const sentimentPercent = ((sentiment.score + 1) / 2 * 100).toFixed(0); // Convert -1 to 1 into 0-100%
            const sentimentLabel = sentiment.score > 0.3 ? '📈 Bullish' : sentiment.score < -0.3 ? '📉 Bearish' : '➡️ Neutral';
            const sentimentClass = sentiment.score > 0.3 ? 'positive' : sentiment.score < -0.3 ? 'negative' : 'neutral';
            
            aiSentimentHtml = `
                <div class="ai-sentiment-section">
                    <button class="ai-sentiment-toggle" onclick="toggleAISentiment('coin-${index}')">
                        <span>✨ AI Analysis</span>
                        <span class="arrow">▼</span>
                    </button>
                    <div id="ai-sentiment-coin-${index}" class="ai-sentiment-content" style="display: none;">
                        <div class="sentiment-header ${sentimentClass}">
                        ...
                        ${sentiment.key_points ? `
                            <div class="sentiment-points">
                                ${sentiment.key_points.map(point => `<div class="sentiment-point">• ${point}</div>`).join('')}
                            </div>
                        ` : ''}
                        ${sentiment.reasoning ? `
                        ...
                        ` : ''}
                        ${coin.sentiment_boost ? `
                            <div class="sentiment-boost">
                                Score boost: ${coin.sentiment_boost > 0 ? '+' : ''}${coin.sentiment_boost.toFixed(1)}
                            </div>
                        ` : ''}
                    </div>
                </div>
...
// Toggle AI sentiment visibility
...
function generateFavoritesTable(favorites, mlEnabled = false) {
    ...
    favorites.forEach((coin, index) => {
        ...
        if (coin.ai_sentiment && coin.ai_sentiment.key_points && coin.ai_sentiment.key_points.length > 0) {
            ...
            aiSentimentHtml = `
                <div class="ai-sentiment-section">
                    <button class="ml-reasoning-toggle" onclick="toggleAISentiment('fav-${index}')">
                        <span>🧠 AI Sentiment (DeepSeek)</span>
                        <span class="arrow">▼</span>
                    </button>
                    <div id="ai-sentiment-fav-${index}" class="ml-reasoning-content" style="display: none;">
                        <div class="ml-prediction-detail">
                            <div class="prediction-row">
